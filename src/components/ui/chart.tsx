...
function ChartTooltipContent(
  props: TooltipContentProps<ValueType, NameType> & ChartTooltipExtraProps,
) {
  const {
    active,
    payload,
          ...
          return (
            ...
            >
              {formatter && item?.value !== undefined && item.dataKey ? (
                formatter(item.value, item.dataKey as NameType, item, index, payload as Payload<ValueType, NameType>[])
              ) : (
                <>
                  {itemConfig?.icon ? (
                    <itemConfig.icon />
...
function ChartLegendContent(props: LegendContentProps & {
  hideIcon?: boolean;
  nameKey?: string;
}) {
  const {
  ...
  if (!payload || !(payload as LegendPayload[]).length) {
    return null;
  }
  ...
  return (
    ...
    >
      {legendPayload.map((item: LegendPayload, index: number) => {
        const key = `${nameKey || item.dataKey || "value"}`;
        const itemConfig = getPayloadConfigFromPayload(config, item, key);
